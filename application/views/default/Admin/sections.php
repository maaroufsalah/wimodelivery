...
$stmt = $con->prepare("SELECT * FROM sections WHERE md5(sec_id) = :sec_id");
$stmt->bindParam(':sec_id', $id, PDO::PARAM_STR);
$stmt->execute();
...
$section_id = $item['sec_id'];  // الاحتفاظ بالمعرف الحقيقي
$name = $item['sec_name'] ?? '';
$note = $item['sec_note'] ?? '';
?>
...
if (!$section) {
echo "<div class='alert alert-danger'>القسم المحدد غير موجود.</div>";
exit();
}
...
JOIN section_products sp ON p.p_id = sp.product_id
WHERE sp.sec_id = :sec_id
");
$stmtProducts->bindParam(':sec_id', $section['sec_id'], PDO::PARAM_INT);
...
<button class="btn btn-danger btn-smdelete-product" data-id="<?= $product['p_id']; ?>" data-sec="<?= $section['sec_id']; ?>">❌ supprimer</button>
</div>
</div>
</div>
<?php endforeach; ?>
<?php else: ?>
<div class="col-12">
<div class="alert alert-warning">Il n'y a aucun produit dans cette section.</div>
</div>
<?php endif; ?>
</div>
</div>
...
echo "<div class='alert alert-danger'>La section n'est pas spécifiée.</div>";
}
?>
...
if (confirm("Voulez-vous supprimer ce produit de la section ?")) {
$.ajax({
url: "delete_product_section",
type: "POST",
