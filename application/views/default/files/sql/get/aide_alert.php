...
$aide = $loginUser['user_aide'] ?? null;

if (!empty($aide)) {
    // عرض الأخبار الخاصة بالمستخدم فقط
    $stmt = $con->prepare("
        SELECT * FROM news
        WHERE n_unlink = '0'
          AND n_rank = 'user'
        ...
        ORDER BY n_id DESC
    ");
    $stmt->execute(['loginId' => $aide]);
} else {
    // عرض الأخبار العامة فقط
    $stmt = $con->prepare("
        SELECT * FROM news
        WHERE n_unlink = '0'
          AND n_rank = 'user'
          AND (n_user IS NULL OR n_user = '')
        ORDER BY n_id DESC
    ");
    $stmt->execute();
}
...
if (count($alerts) > 0) {
    foreach ($alerts as $a_row) {
        if ($a_row['n_type'] == "alert") {
            ...
            ?>
            <!-- Modal Bootstrap -->
            <div class="modal fade" id="autoOpenModal<?=$a_row['n_id'];?>" tabindex="-1" aria-labelledby="autoOpenModalLabel<?=$a_row['n_id'];?>" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">

                        <div class="modal-header">
                            <h5 class="modal-title" id="autoOpenModalLabel<?=$a_row['n_id'];?>">Actualités</h5>
