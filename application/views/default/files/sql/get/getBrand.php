...
$xoo = $loginRank == 'admin' ? " brand_unlink = '0' " : " brand_unlink = '10' ";

$query = "SELECT * FROM $table WHERE $xoo ";
if ($search != '') {
$srs = str_replace(' ', '%', $search);
$query .= "AND (brand_id LIKE '%$srs%' OR brand_name LIKE '%$srs%') ";
}

$query .= "ORDER BY brand_id DESC LIMIT $start, $limit";

// استعلام عدد النتائج
$countStatement = $con->prepare("SELECT COUNT(*) FROM $table WHERE $xoo " . ($search != '' ? "AND (brand_id LIKE '%$srs%' OR brand_name LIKE '%$srs%')" : ""));
...
echo "<div class='col-4'><h6><b>Marques</b></h6></div>";
...
print "<img src='uploads/brand/{$row['brand_image']}' class='m-3' style='width:70px;height:70px;'/>";
}
echo "<h6>" . $row['brand_name'] . "</h6>";
...
if ($loginRank == "admin") {
echo "
<a   data-bs-toggle='modal' data-bs-target='#modalUpdate" . $row['brand_id'] . "'  class='text-info' style='font-size: 26px;'>
<i class='fa-regular fa-pen-to-square'></i>
</a>
";
echo "
<a data-bs-toggle='modal' data-bs-target='#modalDelete" . $row['brand_id'] . "' class='text-danger' style='font-size: 26px;'>
...
if ($loginRank == "admin") {
echo "<div class='modal fade' id='modalDelete" . $row['brand_id'] . "' tabindex='-1' aria-hidden='true'>";
...
echo "<a class='btn btn-success' href='dataUnlink?do=brand&dataUnlinkId=" . md5($row['brand_id']) . "'>Oui, je veux</a>";
echo "</div>";
echo "</div>";
echo "</div>";
echo "</div>";
...
$id = "formId".$row['brand_id'];
$result = "data_result".$row['brand_id'];
$action = "editBrand";
$method = "post";
formAwdStart ($id,$result,$action,$method); 

print "<input type='hidden' name='id' value='" . md5($row['brand_id']) . "'/>";

echo "<div class='modal fade' id='modalUpdate" . $row['brand_id'] . "' tabindex='-1' aria-hidden='true'>";
echo "<div class='modal-dialog modal-dialog-centered'>";
echo "<div class='modal-content'>";
echo "<div class='modal-header'>";
...
<input name="brand" type="text" class="form-control" value="'.$row['brand_name'].'" placeholder=""/>
</div>
</div>
';
