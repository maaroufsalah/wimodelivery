...
$xoo = $loginRank == 'admin' ? " sec_unlink = '0' " : " sec_unlink = '10' ";

$query = "SELECT * FROM $table WHERE $xoo ";
if ($search != '') {
$srs = str_replace(' ', '%', $search);
$query .= "AND (sec_id LIKE '%$srs%' OR sec_name LIKE '%$srs%') ";
}

$query .= "ORDER BY sec_id DESC LIMIT $start, $limit";
...
echo "<div class='col-4'>";
echo "<h6>" . $row['sec_id'] . "</h6>";
echo "</div>";

echo "<div class='col-4'>";
if (empty($row['sec_image'])){
print "<i class='fa-solid fa-camera-retro fa-2x'></i>";
}else{
print "<img src='uploads/sections/{$row['sec_image']}' class='m-3' style='width:70px;height:70px;'/>";
}
echo "<h6>" . $row['sec_name'] . "</h6>";
...
if ($loginRank == "admin") {
echo "
<a data-bs-toggle='modal' data-bs-target='#modalDelete" . $row['sec_id'] . "' class='text-danger' style='font-size: 26px;'>
...
if ($loginRank == "admin") {
echo "
<a href='?do=edit&id=".md5($row['sec_id'])."' class='text-info' style='font-size: 26px;'>
...
if ($loginRank == "admin") {
echo "<div class='modal fade' id='modalDelete" . $row['sec_id'] . "' tabindex='-1' aria-hidden='true'>";
...
echo "<a class='btn btn-success' href='dataUnlink?do=section&dataUnlinkId=" . md5($row['sec_id']) . "'>Oui, je veux</a>";
echo "</div>";
echo "</div>";
echo "</div>";
echo "</div>";
